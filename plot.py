...
def plotWinRate(player_names: list[str], values: list[float], y_range=[0, 0.7]) -> None:
    fig = plt.figure()
    ax = fig.add_axes(axis_rect)
    x_positions = np.arange(0, 3.75, 1.25)
...
def plotBoth(player_names: list[str], win_rates: list[float], loss_reasons: list[float]) -> None:
    plotWinRate(player_names, win_rates)
    plotLossReason(player_names, loss_reasons)
